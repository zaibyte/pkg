...
func (s *hashSet) addS_seed(x string, seed maphash.Seed) {
	var h maphash.Hash
	h.SetSeed(seed)
	h.WriteString(x)
	s.add(h.Sum64())
...
//	s := "hello"
//	for i := 0; i < N; i++ {
//		h.addS_seed(s, maphash.Seed(uint64(i + 1)))
//		h.addS_seed(s, maphash.Seed( uint64(i+1) << 32)) // make sure high bits are used
//	}
//	h.check(t)
//}
